    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-black theme-text-main tracking-tight">Edit Discussion</h1>
            <p class="text-sm theme-text-muted mt-1 font-medium">Update discussion details</p>
          </div>
        </div>
        <div class="flex items-center gap-3">
          <PrimaryButton label="Update Discussion" :loading="saving" @click="saveDiscussion" />
        </div>
      </div>
      ...
      <div class="theme-bg-card border theme-border rounded-[2rem] p-8 shadow-sm space-y-6">
        <form @submit.prevent="saveDiscussion" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormDropdown v-model="form.status" label="Status" :options="statusOptions" :error="errors.status"
              required />
...
const fetchData = async () => {
  ...
  try {
    const [discussion] = await Promise.all([
      discussionService.show(route.params.uuid),
      fetchSelectData()
    ]);
    ...
    Object.assign(form, {
      ...
      type: discussion.data.type,
      status: discussion.data.status,
      course_id: discussion.data.course?.id || '',
      lesson_id: discussion.data.lesson?.id || null,
      user_id: discussion.data.user?.id || '',
      is_featured: !!discussion.data.is_featured,
      is_pinned: !!discussion.data.is_pinned
    });
  } catch (error) {
...
const saveDiscussion = async () => {
  ...
  errors.value = {};
  try {
    await discussionService.update(route.params.uuid, form);
