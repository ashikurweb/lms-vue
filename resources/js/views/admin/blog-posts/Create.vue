  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <button 
          ...
          :disabled="saving"
          class="px-6 py-3 rounded-2xl font-bold theme-bg-card border theme-border theme-text-main hover:theme-bg-sidebar active:scale-95 transition-all outline-none"
        >
          Save Draft
        </button>
        <button 
    ...
    <div class="grid grid-cols-12 gap-8">
      ...
      <div class="col-span-12 lg:col-span-8 space-y-6">
        ...
        <div class="theme-bg-card border theme-border rounded-[2rem] p-8 shadow-sm space-y-6">
          ...
          <div class="space-y-4">
            ...
            <textarea 
              ...
              placeholder="Wait, what's this article about? (Keep it short)"
            ></textarea>
        ...
        <div class="theme-bg-card border theme-border rounded-[2rem] p-8 shadow-sm space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormInput v-model="form.meta_title" label="Meta Title" placeholder="Title for Google search" :error="errors.meta_title" />
            <FormInput v-model="form.meta_keywords" label="Meta Keywords" placeholder="Keyword1, Keyword2" :error="errors.meta_keywords" />
            <div class="md:col-span-2">
               <FormInput v-model="form.canonical_url" label="Canonical URL" placeholder="https://example.com/original-post" :error="errors.canonical_url" />
            </div>
            <div class="md:col-span-2">
      ...
      <div class="col-span-12 lg:col-span-4 space-y-6">
        <!-- Publishing Info -->
        <div class="theme-bg-card border theme-border rounded-[2rem] p-8 shadow-sm space-y-6">
          <h3 class="text-lg font-black theme-text-main tracking-tight border-b theme-border pb-4">Publish & Organize</h3>
          <FormDropdown 
            v-model="form.status"
          ...
          <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              >
                ...
                <div class="px-4 py-2 rounded-xl border-2 theme-border theme-text-dim text-xs font-black peer-checked:bg-indigo-500 peer-checked:border-indigo-500 peer-checked:text-white! hover:theme-border-main transition-all group-active:scale-95">
                  #{{ tag.name }}
                </div>
              </label>
            </div>
          ...
          <div class="space-y-4 theme-bg-element p-4 rounded-2xl border theme-border">
            <div class="flex items-center justify-between">
              ...
              <button @click="form.is_featured = !form.is_featured" class="w-12 h-6 rounded-full relative transition-colors duration-300" :class="form.is_featured ? 'bg-indigo-600' : 'bg-slate-700'">
                <div class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform duration-300" :class="form.is_featured ? 'translate-x-6' : ''"></div>
              </button>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs font-black theme-text-main">Pinned on Top</span>
              <button @click="form.is_pinned = !form.is_pinned" class="w-12 h-6 rounded-full relative transition-colors duration-300" :class="form.is_pinned ? 'bg-indigo-600' : 'bg-slate-700'">
                <div class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform duration-300" :class="form.is_pinned ? 'translate-x-6' : ''"></div>
              </button>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs font-black theme-text-main">Allow Comments</span>
              <button @click="form.allow_comments = !form.allow_comments" class="w-12 h-6 rounded-full relative transition-colors duration-300" :class="form.allow_comments ? 'bg-indigo-600' : 'bg-slate-700'">
                <div class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform duration-300" :class="form.allow_comments ? 'translate-x-6' : ''"></div>
              </button>
            </div>
          ...
          <div class="pt-4">
             <FormInput v-model="form.published_at" type="datetime-local" label="Publish Date" required :error="errors.published_at" />
          </div>
        </div>
        ...
        <div class="theme-bg-card border theme-border rounded-[2rem] p-8 shadow-sm space-y-6">
          ...
          <div class="space-y-6">
            ...
            <FormInput v-model="form.video_url" label="Video URL (YouTube/Vimeo)" placeholder="https://..." :error="errors.video_url" />
          </div>
        </div>
      </div>
    </div>
