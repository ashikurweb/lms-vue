<template>
  <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 relative px-2">
    ...
    <div class="relative space-y-2">
      <h1 class="text-4xl font-black theme-text-main tracking-tighter leading-none flex items-center gap-3">
        <div v-if="$slots.icon || icon"
          class="w-12 h-12 rounded-2xl bg-linear-to-br from-indigo-600 to-violet-600 flex items-center justify-center text-white shadow-lg shadow-indigo-600/20">
          <slot name="icon">
    ...
    <div class="flex items-center gap-4 relative z-10">
      <!-- Search Component -->
      <TableSearch v-if="showSearch" :model-value="modelValue" :placeholder="searchPlaceholder"
        @update:model-value="$emit('update:modelValue', $event)" @search="$emit('search')" />

      <!-- Add Button -->
      <slot name="actions">
        <PrimaryButton v-if="addRoute" variant="primary" :to="addRoute" :label="addLabel"
