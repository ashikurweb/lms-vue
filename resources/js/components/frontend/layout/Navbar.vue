<template>
  <header class="fixed top-6 left-1/2 -translate-x-1/2 z-[150] w-[92%] max-w-[1600px] transition-all duration-700">
    <div
      ...
      <router-link to="/" class="flex items-center gap-2 md:gap-3 group shrink-0">
        <div
          class="w-8 h-8 md:w-10 md:h-10 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-600/20 group-hover:rotate-12 transition-all">
          <span class="text-white font-black text-lg md:text-xl">N</span>
        </div>
        <div class="flex flex-col -space-y-1">
          <span
            class="text-lg md:text-xl font-black theme-text-main tracking-tighter uppercase italic leading-none">Nexus<span
              class="text-indigo-600 not-italic">LMS</span></span>
          <span
            class="text-[8px] font-black theme-text-dim uppercase tracking-[0.3em] pl-1 hidden sm:block">Engineering</span>
        </div>
      </router-link>
      ...
      <div class="flex items-center gap-2 md:gap-5">
        <button @click="toggleTheme"
          class="w-9 h-9 md:w-10 md:h-10 flex items-center justify-center rounded-xl hover:theme-bg-hover transition-all border theme-border theme-bg-element shadow-sm shrink-0">
          <svg v-if="theme === 'dark'" class="w-4 h-4 md:w-5 md:h-5 text-amber-400" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
          ...
          <svg v-else class="w-4 h-4 md:w-5 md:h-5 text-indigo-600" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
        ...
        <router-link v-if="!isAuthenticated" to="/login"
          class="text-[10px] font-black theme-text-main uppercase tracking-widest hover:text-indigo-600 transition-colors hidden md:block text-nowrap">Login</router-link>
        ...
        <div v-else class="relative hidden md:block">
          <button @click="isProfileOpen = !isProfileOpen"
            ...
            <div
              class="w-7 h-7 rounded-xl overflow-hidden border theme-border group-hover:border-indigo-500/30 transition-all">
              <img :src="`https://ui-avatars.com/api/?name=${user?.name || 'User'}&background=6366f1&color=fff`"
                alt="User" class="w-full h-full object-cover">
            </div>
            <span
              class="text-[10px] font-black theme-text-main group-hover:text-indigo-600 transition-colors uppercase tracking-widest">{{
                user?.name?.split(' ')[0] || 'User' }}</span>
            <svg class="w-3.5 h-3.5 theme-text-dim transition-transform duration-300"
          ...
          <transition name="dropdown">
            <div v-if="isProfileOpen"
              ...
              <div class="px-5 py-4 border-b theme-border mb-2">
                ...
                <p class="text-[9px] theme-text-dim font-bold uppercase tracking-widest mt-1 opacity-70">{{ user?.email
                }}</p>
              </div>
              ...
              <div class="px-2 space-y-1">
                ...
                <router-link v-if="isAdmin" :to="{ name: 'dashboard' }"
                  ...
                  <div
                    class="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center group-hover:bg-indigo-500 group-hover:text-white transition-colors">
                ...
                <router-link v-else :to="{ name: 'student.quizzes' }"
                  class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl theme-text-muted theme-bg-hover hover:theme-text-main group"
                  @click="isProfileOpen = false">
                  <div
                    class="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center group-hover:bg-indigo-500 group-hover:text-white transition-colors">
                  ...
                  <span class="text-xs font-black uppercase tracking-widest">My Quizzes</span>
                </router-link>
                ...
                <router-link
                  ...
                  @click="isProfileOpen = false">
                  <div
                    class="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center group-hover:bg-emerald-500 group-hover:text-white transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button
                  class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl theme-text-muted theme-bg-hover hover:theme-text-main group">
                  <div
                    class="w-8 h-8 rounded-lg bg-amber-500/10 flex items-center justify-center group-hover:bg-amber-500 group-hover:text-white transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div class="px-2 py-2 mt-2 border-t theme-border pt-3">
                <button @click="handleLogout"
                  ...
                  <div
                    class="w-8 h-8 rounded-lg bg-rose-500/20 flex items-center justify-center group-hover:bg-white group-hover:text-rose-500 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <button @click="$emit('toggle-mobile-menu')"
          ...
          <div class="relative w-5 h-5 md:w-6 md:h-6">
            <transition name="icon-pop">
              <svg v-if="!isMobileMenuOpen" class="absolute inset-0 w-full h-full text-indigo-500" fill="none"
                stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 6h16M4 12h16m-7 6h7" />
              </svg>
              <svg v-else class="absolute inset-0 w-full h-full text-indigo-500" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
...
.dropdown-enter-from {
  opacity: 0;
  transform: translateY(15px) scale(0.9) rotateX(-15deg);
}
